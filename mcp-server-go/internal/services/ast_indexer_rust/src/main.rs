...
const HTML_LAYOUT_TAGS: &[&str] = &[
    "html", "body", "main", "section", "article", "aside", "nav", "header", "footer", "form",
    "dialog",
];

fn capture_kind(capture_name: &str) -> Option<&'static str> {
    match capture_name {
...
fn is_scope_container(node_kind: &str) -> bool {
    matches!(
        node_kind,
        "class_definition"
            | "class"
...
fn is_scope_name_kind(node_kind: &str) -> bool {
    matches!(
        node_kind,
        "identifier" | "type_identifier" | "name" | "field_identifier" | "tag_name"
    )
}
...
fn html_symbol_type(name: &str, snippet: &str) -> Option<&'static str> {
    ...
    if HTML_LAYOUT_TAGS.contains(&normalized.as_str()) || snippet.to_ascii_lowercase().contains(" id=")
    {
        return Some("layout");
    }
    None
...
fn css_symbol_type(node_kind: &str) -> Option<&'static str> {
    match node_kind {
        "rule_set" => Some("selector"),
        "keyframes_statement" => Some("keyframes"),
...
fn run_indexer(args: &Args, heartbeat_path: &Path) -> anyhow::Result<()> {
    ...
    let producer_handle = std::thread::spawn(move || {
        ...
        entries_arc.par_iter().for_each(|path| {
            ...
            let project_prefix = project_root.replace("\\", "/");
            let mut rel_path = path_str.clone();
            if rel_path.starts_with(&project_prefix) {
                rel_path = rel_path.replace(&project_prefix, "");
            }
            if rel_path.starts_with('/') {
                rel_path = rel_path[1..].to_string();
            }

            if let Some(old) = db_files_arc.get(&rel_path) {
    ...
    let project_prefix = args.project.clone().replace("\\", "/");

    // Process results
    let mut rx_iter = rx_chan.into_iter();
    while let Some(res) = rx_iter.next() {
        processed_count += 1;

        let mut file_path = res.file_path.clone();
        if file_path.starts_with(&project_prefix) {
            file_path = file_path.replace(&project_prefix, "");
        }
        ...
        for sym in &res.symbols {
            let prefix = match sym.symbol_type.as_str() {
...
fn run_query(args: &Args) -> anyhow::Result<()> {
    ...
    // 🧹 统一后置处理：将数据库中读取的相对路径在内存中恢复为绝对路径
    let project_prefix = args.project.clone().replace("\\", "/");
    let resolve_path = |p: &str| -> String {
        if p.is_empty() { return p.to_string(); }
        if project_prefix.ends_with('/') {
            format!("{}{}", project_prefix, p)
        } else {
            format!("{}/{}", project_prefix, p)
        }
    };

    if let Some(ref mut sym) = found {
        sym.file_path = resolve_path(&sym.file_path);
    }
    for cand in &mut candidates {
        cand.node.file_path = resolve_path(&cand.node.file_path);
    }
    for info in &mut related {
        info.node.file_path = resolve_path(&info.node.file_path);
    }
...
fn run_map(args: &Args) -> anyhow::Result<()> {
    ...
    if let Some(scope) = &args.scope {
        if !scope.is_empty() {
            ...
            let project_prefix = args.project.replace("\\", "/");
            let rows = stmt.query_map([&pattern], |row| {
                let rel_path: String = row.get(0)?;
                let full_path = if project_prefix.ends_with('/') {
                    format!("{}{}", project_prefix, rel_path)
                } else {
                    format!("{}/{}", project_prefix, rel_path)
                };
...
fn run_analyze(args: &Args) -> anyhow::Result<()> {
    ...
    target.file_path = resolve_path(&target.file_path);
    for c in &mut direct_nodes {
        c.node.file_path = resolve_path(&c.node.file_path);
    }
    for c in &mut indirect_nodes {
        c.node.file_path = resolve_path(&c.node.file_path);
    }
...
mod tests {
    ...
    #[test]
    fn html_symbol_type_assigns_semantic_roles() {
        assert_eq!(html_symbol_type("main", "<main>"), Some("layout"));
        assert_eq!(
            html_symbol_type("my-card", "<my-card></my-card>"),
            Some("component")
        );
        assert_eq!(
            html_symbol_type("div", r#"<div id="app"></div>"#),
            Some("layout")
        );
        assert_eq!(html_symbol_type("template", "<template></template>"), Some("template"));
        assert_eq!(html_symbol_type("slot", "<slot></slot>"), Some("slot"));
        assert_eq!(html_symbol_type("div", "<div></div>"), None);
        assert_eq!(html_symbol_type("span", "<span></span>"), None);
    }

    #[test]
    fn parser_setup_registers_html_extensions() {
        let setup = get_parser_setup();
        assert!(setup.contains_key("html"));
        assert!(setup.contains_key("htm"));
        assert!(setup.contains_key("css"));
    }

    #[test]
    fn html_query_extracts_structural_symbols() {
        let setup = get_parser_setup();
        ...
        for m in matches {
            ...
            if let (Some(name), Some(kind), Some(full_node)) = (node_name, node_type, def_node) {
                let snippet = &source[full_node.start_byte()..full_node.end_byte()];
                if kind == "component" && html_symbol_type(&name, snippet).is_some() {
                    names.push(name);
                }
            }
        }
        ...
        assert!(names.iter().any(|name| name == "my-card"));
        assert!(!names.iter().any(|name| name == "div"));
    }

    #[test]
    fn html_query_resolves_semantic_symbol_types() {
        let setup = get_parser_setup();
        let (lang, query) = setup.get("html").expect("html parser setup missing");
        ...
        for m in matches {
            ...
            if let (Some(name), Some(full_node)) = (node_name, def_node) {
                let snippet = &source[full_node.start_byte()..full_node.end_byte()];
                if let Some(kind) = html_symbol_type(&name, snippet) {
                    resolved.insert(name, kind.to_string());
                }
            }
        }

        assert_eq!(resolved.get("body").map(String::as_str), Some("layout"));
        assert_eq!(resolved.get("template").map(String::as_str), Some("template"));
        assert_eq!(resolved.get("slot").map(String::as_str), Some("slot"));
        assert_eq!(resolved.get("my-panel").map(String::as_str), Some("component"));
    }

    #[test]
    fn css_query_extracts_selectors_and_keyframes() {
        let setup = get_parser_setup();
        ...
        for m in matches {
            ...
            if let (Some(name), Some(full_node)) = (node_name, def_node) {
                if let Some(kind) = css_symbol_type(full_node.kind()) {
                    found_types.push((name.trim().to_string(), kind.to_string()));
                }
            }
        }
