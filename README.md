...
LSP 是给 IDE 用的。MPM 是给 AI 用的。需求完全不同。

| | LSP | MPM (Tree-sitter) |
|---|---|---|
| 启动代价 | 每个 Server 几秒到几十秒，还要 index workspace | 毫秒级解析，无预热 |
| 依赖 | 每种语言一个 Server，背后是编译器生态 | 单二进制，零外部依赖 |
| 项目要求 | 能编译才能分析 | 源码在就能解析，不依赖构建系统 |
| 部署 | 需要管理多个长驻进程 | 一个 MCP Server，开箱即用 |
| AI 需要的 | 符号定义、调用链、结构——不需要补全和重构 | Tree-sitter 精确提供这些 |

AI 编程助手的核心瓶颈是**注意力**——它不知道该看哪里。AST 索引做的就是把 "该看哪里" 这个问题从猜测变成确定性的符号查询。LSP 提供的代码补全、跳转定义、重命名，AI 客户端自己就有，不需要 MPM 再做一遍。
