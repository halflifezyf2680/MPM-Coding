...
这里不是讲工具清单，而是先讲 MPM 的工作方式。可以把它理解成三件事：

1. 把流程固定下来（先定位、先影响分析、再修改）
2. 把任务状态固定下来（长任务可分段、可验收、可恢复）
3. 把决策记录固定下来（为什么这么改，后面能追溯）

对应到实际落地，是下面这些核心概念：

| 核心概念 | 一句话说明 | 主要工具 | 落地结果 |
|------|------|---------|---------|
| **项目锚点** | 先把当前会话绑定到正确项目根 | `initialize_project` | 避免在错误目录写数据/建索引 |
...
MPM 用 Rust AST 引擎解析代码，并把下面这些信息落到本地数据库里：
...
**为什么重要**：同名函数不再靠猜，`code_impact` 才能把调用链追得更准。
