...
Rust AST 索引器（rayon 并行 + tree-sitter），增量索引仅解析变更文件。25,000 文件 / 160,000+ 符号的大型 Go 项目全量索引约 4 分钟；13,000+ 文件的 C++ 项目 35 秒完成。Go 项目耗时较高是因为跨包调用密集，生成的调用边数量远超 C++ 项目（Kubernetes 81 万边 vs Godot 35 万边），链接阶段随边数线性增长。以上数据在 i5-1230U 低功耗处理器上测得，正常桌面处理器预计快 2-3 倍。
