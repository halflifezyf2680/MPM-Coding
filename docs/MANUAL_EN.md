...
6) Multi-layered path isolation (🆕 Major v2.0 update)
- Strip absolute path prefixes ➡️ Persistence using ONLY relative paths in SQLite
- In-memory dynamic assembly in Outbound DTOs with reconstructed absolute paths
- Result: Highly scalable, container-ready, securely containerized, and fully portable setup.
...
| 5 | `code_search` | Perception | AST precise symbol locator |
| 6 | `code_impact` | Perception | Call chain impact analysis |
| 7 | `task_chain` | Scheduling | Protocol state machine task chain management |
| 8 | `system_hook` | Scheduling | Todo hook management (create/list/release) |
...
| `description` | string | init/update | Task description |
...
| `phases` | array | init/update | Custom phase definitions |

**Modes**:
| Mode | Description |
|------|-------------|
| `init` | Initialize task chain with protocol |
| `update` | Modify task goal (preserves completed phases) |
| `start` | Begin a phase |
...
| `develop` | implement (execute) → verify_gate (gate) → finalize (execute) | Feature development with verification |
| `debug` | fix (execute) → verify_gate (gate) → finalize (execute) | Bug investigation and fix |
| `refactor` | refactor (execute) → verify_gate (gate) → finalize (execute) | Code refactoring with verification |

**Example**:
```javascript
// 1. Initialize a development task
task_chain(mode="init", task_id="AUTH_FEATURE", protocol="develop", description="Add OAuth2 support")

// 2. Complete implementation phase
task_chain(mode="complete", task_id="AUTH_FEATURE", phase_id="implement", summary="OAuth2 flow implemented, basic tests pass")

// 3. Pass verification gate
task_chain(mode="complete", task_id="AUTH_FEATURE", phase_id="verify_gate", result="pass", summary="All integration tests pass")

// 4. Finalize with memo record
task_chain(mode="complete", task_id="AUTH_FEATURE", phase_id="finalize", summary="Feature complete, memo recorded")
```
...
- **Adaptive adjustment**: Use `update` mode to modify task goal when deviation found, completed phases preserved
...
| Java | .java |
| C/C++ | .c, .cpp, .h, .hpp |
| HTML | .html, .htm (structure symbols: layout/template/slot/component, no call graph yet) |
