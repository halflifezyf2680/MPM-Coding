...
| 11 | `known_facts` | 记忆 | KnownFact 策略引擎 |
...
KnownFact 策略引擎。旧式 `type + summarize` 保存仍兼容；新主路径是行动前召回经验、行动后回写结果。
...
- `mode=status`：查看事实状态。
- `mode=maintain`：查看收敛维护建议。

**`new_observations` 写入指引**：只写可复用经验（格式："在XX条件下应该/不应该YY"），不写任务完成确认（如"Successfully fixed..."）或操作流水账。持久化到 `.claude/CLAUDE.md` 和 `AGENTS.md`，adopted 条目不落盘。
