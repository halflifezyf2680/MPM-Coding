...
6) 增量分层路径安全隔离 (🆕 v2.0 重大更新)
- 剥离物理绝对路径前缀 ➡️ 底层 100% 采用 SQLite 相对路径持久化
- 系统出口管道动态流向内存 ➡️ 按需复原绝对路径数据包 DTO
- 结果：高移植性、支持 Docker 容器部署，防爆，彻底的安全隔离。
...
| `description` | string | init/update | 任务描述 |
...
| `phases` | array | init/update | 自定义阶段定义 |

**操作模式**：
| 模式 | 描述 |
|------|------|
| `init` | 初始化任务链（指定协议） |
| `update` | 修改任务目标（保留已完成阶段） |
| `start` | 开始一个阶段 |
...
| `develop` | implement (execute) → verify_gate (gate) → finalize (execute) | 功能开发 + 验收 |
| `debug` | fix (execute) → verify_gate (gate) → finalize (execute) | Bug 排查与修复 |
| `refactor` | refactor (execute) → verify_gate (gate) → finalize (execute) | 代码重构 + 验收 |

**示例**：
```javascript
// 1. 初始化一个开发任务
task_chain(mode="init", task_id="AUTH_FEATURE", protocol="develop", description="新增 OAuth2 支持")

// 2. 完成实现阶段
task_chain(mode="complete", task_id="AUTH_FEATURE", phase_id="implement", summary="OAuth2 流程已实现，基础测试通过")

// 3. 通过验收门控
task_chain(mode="complete", task_id="AUTH_FEATURE", phase_id="verify_gate", result="pass", summary="所有集成测试通过")

// 4. 收尾并记录 memo
task_chain(mode="complete", task_id="AUTH_FEATURE", phase_id="finalize", summary="功能完成，memo 已记录")
```
...
- **自适应调整**：发现偏差时用 `update` mode 修改任务目标，保留已完成阶段
...
| Java | .java |
| C/C++ | .c, .cpp, .h, .hpp |
| HTML | .html, .htm（结构符号：layout/template/slot/component，暂不进入调用图）|
| CSS | .css（结构符号：selector/keyframes，不进入调用图）|
